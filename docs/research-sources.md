...
Checked on 25 September 2026. Public primary sources were preferred. No private External platform tenant, commercial contract, internal database or source implementation was inspected.

## Primary register

| Source | What it supports | Evidence boundary |
|---|---|---|
| [External platform platform](urn:chargeweave:legacy-site/ev-charging-platform/) | Product scope extends across operations, customer experience, energy, commercial services and integrations | Vendor product positioning; not independent performance verification |
| [API functional components](urn:chargeweave:legacy-api/docs/main-players) | Vendor actors and resource context | Used as a benchmark, not copied as canonical naming |
| [Models relationship](urn:chargeweave:legacy-api/docs/models-relationship) | Relationships in the public resource model | Canonical design further separates identity, versions and evidence |
| [API reference](urn:chargeweave:legacy-api/reference/authorization-1) | Full browser-visible navigation, displayed v3.253.0 | Raw navigation facts retained; one linked OAuth heading excluded from operation count |
| [Public OpenAPI specification](urn:chargeweave:legacy-api/openapi/public-api.yaml) | Current header version and authentication contract | Read through web retrieval; complete raw-schema download was not obtained |
| [Official API exploration description](urn:chargeweave:legacy-api/reference/api-ai-skill) | Identifies External platform's official public schema tooling | Used as documentation, not installed or executed |
| [Official schema index](https://github.com/external-platform/claude-code-external-platform-public-api-skill/blob/main/reference/schemas-index.md) | 971 named schema entries and property inventories, v3.251.8 | Older version than live navigation; generic schema names cannot be treated as self-explanatory business classes |
| [Official endpoint index](https://github.com/external-platform/claude-code-external-platform-public-api-skill/blob/main/reference/endpoints-index.md) | 651 operations, including OAuth paths, v3.251.8 | Equal counts do not establish equality of versioned endpoint sets |
| [Tariff create](urn:chargeweave:legacy-api/reference/tariffcreate) | Detailed pricing and discount structures | All inspected fields have semantic dispositions; numeric equivalence to External platform calculations is not proven |
| [Charge point update](urn:chargeweave:legacy-api/reference/chargepointupdate) | Personal/commercial distinctions, rate dependencies and separate monitoring/recovery controls | Deprecated and experimental behavior requires adapter version pinning |
| [Circuit create](urn:chargeweave:legacy-api/reference/circuitcreate) | Multilevel load-management restrictions and electrical context | Some restrictions are vendor compatibility policy rather than universal electrical ontology truths |
| [Notifications subscribe](urn:chargeweave:legacy-api/reference/notificationssubscribe) | Event subscription concepts and observed callback identifiers | Captured callback list is versioned, not presumed permanent |
