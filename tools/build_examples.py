...
# Nonzero financial facts make payment predicates meaningful in the full fixture.
dec('PaymentIntent','requestedAmount','1');dec('PaymentCapture','capturedAmount','1')
code('PaymentIntent','paymentState','Captured')
code('PaymentCapture','captureState','Confirmed');dec('Refund','refundAmount','1');dec('Chargeback','disputedAmount','1')
dec('CreditNote','creditAmount','1')
for c in ['Invoice','InvoiceLine']:
 dec(c,'netAmount','1');dec(c,'grossAmount','1')
setv('OwnershipTransfer','nextOwner',node('LegalEntity'))
setv('PricingTimeResolution','timezoneName',Literal('Europe/Amsterdam',datatype=XSD.string))
setv('PricingTimeResolution','sourceLocalTime',Literal('2026-09-25T14:00:00+02:00',datatype=XSD.dateTime))
setv('PricingTimeResolution','resolvedUtcTime',Literal('2026-09-25T12:00:00Z',datatype=XSD.dateTime))
