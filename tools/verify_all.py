...
for args in [['validate.py','--meta'],['test_business.py'],['test_boundaries.py'],['test_trust_boundary.py'],['check_owl.py'],['check_release.py'],['check_audit.py'],['test_business_acceptance.py'],['test_lifecycle.py'],['check_naming.py'],['check_completeness.py'],['test_calculation_oracles.py']]:
...
for family in ('access','finance','pricing','settlement','governance','lifecycle','operations','financial-state'):
 subprocess.run([sys.executable,str(P/'test_completion.py'),'--family',family],check=True)
