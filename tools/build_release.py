...
manifest={'release':'1.2.0','date':'2026-09-25','namespace':str(C),'counts':{'domainClasses':len(catalog),'domainProperties':len(types),'modules':len({x['module'] for x in catalog.values()}),'codeSchemes':sum(1 for x in g.subjects(RDF.type,Namespace('http://www.w3.org/2004/02/skos/core#').ConceptScheme)),'businessRules':len(json.loads((P/'model/rules.json').read_text())),'resourceFamilies':len(families),'operationPages':len(ops),'uniqueOperationURLs':len({x['source'] for x in ops}),'ontologyTriples':len(g),'competencyQueries':len(list((P/'queries').glob('CQ*.rq')))},'dependencies':{x:importlib.metadata.version(x) for x in ['rdflib','pyshacl','owlready2']},'evidenceBoundary':'Declared CPMS semantic baseline with reviewed concepts, journeys and executable evidence; legacy catalogue traceability is separate from completeness and runtime certification.','files':{}}
